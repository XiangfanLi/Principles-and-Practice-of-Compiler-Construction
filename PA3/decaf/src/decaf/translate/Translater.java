...
public class Translater {
	...
	public Temp genIndexedDefault(Temp array, Temp index, Temp default_val) {
		Temp length = genLoad(array, -OffsetCounter.WORD_SIZE);
		Temp zero = genLoadImm4(0);
		Temp obj;
		Temp cond1 = genLes(index, length);
		Temp cond2 = genLes(index, zero);
		Label err = Label.createLabel();
		Label exit = Label.createLabel();
		genBeqz(cond1, err);
		genBnez(cond2, err);
	...
	public void genCheckArrayRepeatTimes(Temp size) {
		Label exit = Label.createLabel();
		Temp cond = genLes(size, genLoadImm4(0));
		genBeqz(cond, exit);
		Temp msg = genLoadStrConst(RuntimeError.NEGATIVE_ARRAYREPEAT_TIMES);
		genParm(msg);
		genIntrinsicCall(Intrinsic.PRINT_STRING);
		genIntrinsicCall(Intrinsic.HALT);
		genMark(exit);
	...
	public Temp genArrayRepeat(Type type, Temp element, Temp length) {
		genCheckArrayRepeatTimes(length);
		...
		if(type.isClassType())
		{
			...
			Temp clone = Temp.createTempI4();
			genOCStmt(((ClassType)(type)).getSymbol(),clone,element);
			genStore(clone,obj,0);
			
		}
		else {
			genStore(element, obj, 0);
		}
		genBranch(loop);
		genMark(exit);
	...
	public Temp genLoadArrayConst(List<Expr> lst) {
		Temp unit = genLoadImm4(OffsetCounter.WORD_SIZE);
		Temp length = genLoadImm4(lst.size());
	...
	public void genOCStmt(Class c, Temp dst, Temp src) {
		Temp size = genLoadImm4(c.getSize());
		
		genParm(size);
		genAssign(dst, genIntrinsicCall(Intrinsic.ALLOCATE));
		int time = c.getSize() / OffsetCounter.WORD_SIZE - 1;
		if (time != 0) {
			Temp zero = genLoadImm4(0);
				for (int i = 0; i < time; i++) {
					genStore(genLoad(src, OffsetCounter.WORD_SIZE * (i + 1)), dst, OffsetCounter.WORD_SIZE * (i + 1));
				}
		}
		
		genStore(genLoad(src, 0), dst, 0);
