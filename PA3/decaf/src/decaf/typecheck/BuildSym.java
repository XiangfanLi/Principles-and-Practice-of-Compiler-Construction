...
public class BuildSym extends Tree.Visitor {
	...
	public void visitTopLevel(Tree.TopLevel program) {
		...
		for(Tree.ClassDef cd : program.classes) {
			...
			if(cd.parent != null )
			{
				for(Tree.ClassDef pr : program.classes)
				{
					if(pr.name.equals(cd.parent))
					{
						if(pr.sealed != null)
						{
	...
	public void visitAssign(Tree.Assign assign) {
		...
		if(assign.left instanceof Tree.VarIdent) {
			String viname = ((Tree.VarIdent)(assign.left)).name;
			Symbol sym = table.lookup(viname, true);
			if (sym != null) {
				if (table.getCurrentScope().equals(sym.getScope())) {
					issueError(new DeclConflictError(assign.left.getLocation(), viname,
							sym.getLocation()));
	...
	public void visitForeachStmt(Tree.ForeachStmt foreachStmt) {
		foreachStmt.scopeBlock.associatedScope = new LocalScope(foreachStmt.scopeBlock);
		table.open(foreachStmt.scopeBlock.associatedScope);
		for(Tree t : foreachStmt.scopeBlock.block) {
			t.accept(this);
		}
		table.close();
	}
	...
	public void visitBoundVariable(Tree.BoundVariable boundVariable) {
		...
		Variable v = new Variable(boundVariable.ident, boundVariable.typel.type, boundVariable.getLocation());
		
		table.declare(v);
		boundVariable.symbol = v;
