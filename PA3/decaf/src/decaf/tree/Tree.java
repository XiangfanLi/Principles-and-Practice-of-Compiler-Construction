...
import decaf.tree.Tree.NewClass;
import decaf.tree.Tree.Null;
import decaf.tree.Tree.OCStmt;
import decaf.tree.Tree.Print;
import decaf.tree.Tree.ReadIntExpr;
import decaf.tree.Tree.ReadLineExpr;
import decaf.tree.Tree.Return;
import decaf.tree.Tree.Skip;
import decaf.tree.Tree.ThisExpr;
import decaf.tree.Tree.TopLevel;
import decaf.tree.Tree.TypeArray;
import decaf.tree.Tree.TypeCast;
import decaf.tree.Tree.TypeClass;
import decaf.tree.Tree.TypeIdent;
import decaf.tree.Tree.TypeLiteral;
import decaf.tree.Tree.TypeTest;
import decaf.tree.Tree.Unary;
import decaf.tree.Tree.VarDef;
import decaf.tree.Tree.VarIdent;
import decaf.tree.Tree.Visitor;
import decaf.tree.Tree.WhileLoop;
...
public abstract class Tree {
    ...
    public static final int ARRAYCONST = PRINT + 1;
    public static final int VARIDENT = ARRAYCONST + 1;
    public static final int IFSUBSTMT = VARIDENT + 1;
    public static final int GUARDSTMT = IFSUBSTMT + 1;
    public static final int ARRAYCONSTANT = GUARDSTMT + 1;
    public static final int ARRAYREPEAT = ARRAYCONSTANT + 1;
    public static final int ARRAYCONCAT = ARRAYREPEAT + 1;
    public static final int VAR = ARRAYCONCAT + 1;
    public static final int BOUNDVARIABLE = VAR + 1;
    public static final int FOREACHSTMT = BOUNDVARIABLE +1;
    public static final int SUBARRAY = FOREACHSTMT + 1;
    public static final int INDEXEDDEFAULT = SUBARRAY + 1;
    public static final int PYARRAY = INDEXEDDEFAULT + 1;
    public static final int OCSTMT = PYARRAY + 1;
    ...
    public static class ForeachStmt extends Tree {
    	...
    	public ForeachStmt(BoundVariable boundVariable, Expr array, Expr condition, Tree stmt, Location loc)
    	{
    		...
    		lst.add(boundVariable);
    		lst.add(array);
    		lst.add(condition);
    		if(stmt instanceof Block) {
    			for(Tree tree : ((Block)stmt).block) {
    				lst.add(tree);
    			}
    		}
    		else
    			lst.add(stmt);
    		scopeBlock = new Block(lst, this.loc);
    	}
    ...
    public static class BoundVariable extends Expr {
    	public TypeLiteral typel;
    	public String ident;
    	public Variable symbol;
    	public BoundVariable(TypeLiteral typel, String ident, Location loc){
    ...
    public static class GuardStmt extends Tree {
    	public List<IfSubStmt> lst;
    	public GuardStmt(List<IfSubStmt> lst, Location loc){
    		super(GUARDSTMT, loc);
    		this.lst = lst;
    		
    	}
    	public void Add(IfSubStmt i){
    		lst.add(i);
    	}
    ...
    public static class ClassDef extends Tree {
    	...
    	public Class symbol;
    	 ...
    	 public ClassDef(String sealed, String name, String parent, List<Tree> fields,
     			Location loc) {
     		super(CLASSDEF, loc);
     		this.sealed = sealed;
     		this.name = name;
    	...
    	@Override
    	public void printTo(IndentPrintWriter pw) {
    		pw.println((sealed !=null ? sealed+" " : "") + "class " + name + " "
    ...
    public static class OCStmt extends Tree {
    	public Ident ident;
    	public Expr expr;
    	public Location ident_loc;
    	public Symbol dst;
    	public OCStmt(Ident ident, Expr expr, Location loc1, Location loc2) {
    		super(OCSTMT, loc1);
    		this.ident = ident;
    		this.expr = expr;
    		this.type = BaseType.VOID;
    		this.ident_loc = loc2;
    	}
    ...
    public static class ArrayConstant extends Expr {
    	public List<Expr> lst;
    	public ArrayConstant(List<Expr> lst, Location loc){
    ...
    public static class PyArray extends Expr {
    	Expr gen_expr;
    	String ident;
    	Expr array;
    	Expr condition;
    	public PyArray(Expr gen_expr, String ident, Expr array, Expr condition, Location loc) {
    		super(PYARRAY, loc);
    ...
    public static class IndexedDefault extends Expr {
  	   public Expr array;
  	   public Expr index;
  	   public Expr default_val;
  	   public IndexedDefault(Expr array, Expr index, Expr default_val, Location loc) {
  		   super(INDEXEDDEFAULT, loc);
  		   this.array = array;
  		   this.index = index;
  	   ...
  	   @Override
  	   public void accept(Visitor visitor) {
  		   visitor.visitIndexedDefault(this);
  	   }
     ...
     public static class SubArray extends Expr {
  	   public Expr array;
     	   public Expr index1;
     	   public Expr index2;
  	   public SubArray(Expr array, Expr index1, Expr index2, Location loc){
  		   super(SUBARRAY, loc);
  		   this.array = array;
  		   this.index1 = index1;
  		   this.index2 = index2;
    ...
    public static class ArrayConst extends Tree{
    	public List<Expr> lst;
    	public ArrayConst(List<Expr> _lst, Location loc){
...
public static class TypeIdent extends TypeLiteral {
    ...
    public static abstract class Visitor {
        ...
        public void visitOCStmt(OCStmt that) {
        	visitTree(that);
        }
        ...
        public void visitIndexedDefault(IndexedDefault that) {
        	visitTree(that);
        }
